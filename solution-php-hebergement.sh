...
# Script de pru00e9paration pour l'hu00e9bergement mutualisu00e9 avec PHP
echo "Pru00e9paration d'une solution PHP pour Alphas..."

# Cru00e9er le dossier de destination
DEST_DIR="./alphas-php-solution"
mkdir -p "$DEST_DIR"

# Reconstruire l'application avec la configuration standard
...
// Mapper les routes dynamiques
$dynamic_routes = [
    '/produits/([^/]+)' => '/produits/[id]',
    '/admin/produits/([^/]+)' => '/admin/produits/[id]'
];

$html_path = '';

// Vu00e9rifier si c'est une route dynamique
foreach ($dynamic_routes as $pattern => $template) {
...
// Si ce n'est pas une route dynamique, utiliser le chemin directement
if (empty($html_path)) {
    $html_path = $path;
    if ($path === '/') {
        $html_path = '/index';
...
Du00e9ploiement du site Alphas sur hu00e9bergement mutualisu00e9 (Solution PHP)
==================================================================
...
3. Assurez-vous que les fichiers .htaccess et index.php sont bien u00e0 la racine.
...
Remarque : Cette solution utilise PHP pour servir les pages statiques gu00e9nu00e9ru00e9es
par Next.js. Les fonctionnalitu00e9s dynamiques comme les routes API ne fonctionneront pas
complu00e8tement, mais les pages de base du site seront accessibles.
EOL
